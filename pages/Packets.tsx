...
// Map category string → tailwind classes
...
const Packets: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-12 text-slate-400">
        Loading packets…
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-12">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            Technical deep dives and real-world knowledge transfers pulled from the
            Packet Pals database.
          </p>
        </div>
      ...
      {filteredPackets.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredPackets.map((packet) => (
            <article
              key={packet.id}
              className="flex flex-col bg-slate-900 border border-slate-800 rounded-xl overflow-hidden hover:border-cyan-400/60 transition-all duration-300 group h-full"
            >
              {/* Banner */}
              {packet.cover_image_url && (
                <div className="h-44 w-full overflow-hidden relative">
                  ...
                  <div className="absolute top-4 right-4 z-20">
                    <span
                      className={`text-xs font-bold px-2 py-1 rounded uppercase tracking-wide border backdrop-blur-md ${getCategoryStyle(
                        packet.category
                      )}`}
                    >
                      {packet.category}
              ...
              <div className="p-6 flex-grow flex flex-col">
                {!packet.cover_image_url && (
                  <div className="flex justify-between items-start mb-4">
                    <span
                      className={`text-xs font-bold px-2 py-1 rounded uppercase tracking-wide border ${getCategoryStyle(
                        packet.category
                      )}`}
                    >
                      {packet.category}
                ...
                <Link to={`/packets/${packet.id}`} className="block mb-3">
                  <h2 className="text-xl font-bold text-slate-100 group-hover:text-cyan-300 transition-colors line-clamp-2">
                    {packet.title}
                  </h2>
                </Link>
