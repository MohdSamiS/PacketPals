...
const SubnetCalculatorTool = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="sm:col-span-2">
          ...
          <input
            ...
            className="w-full bg-slate-800 border border-slate-700 text-white px-4 py-2 rounded focus:border-brand-accent outline-none font-mono"
      ...
      {result ? (
        <div className="bg-slate-900 rounded border border-slate-800 p-4 space-y-3 font-mono text-sm">
          <div className="flex justify-between border-b border-slate-800 pb-2">
            <span className="text-brand-muted">Subnet Mask</span>
            <span className="text-white">{result.mask}</span>
          </div>
          <div className="flex justify-between border-b border-slate-800 pb-2">
            <span className="text-brand-muted">Network Address</span>
            <span className="text-brand-net">{result.network}</span>
          </div>
          <div className="flex justify-between border-b border-slate-800 pb-2">
            <span className="text-brand-muted">Broadcast Address</span>
            <span className="text-brand-cloud">{result.broadcast}</span>
          </div>
           <div className="flex justify-between border-b border-slate-800 pb-2">
            <span className="text-brand-muted">Host Range</span>
            <span className="text-white">{result.first} - {result.last}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-brand-muted">Usable Hosts</span>
            <span className="text-brand-accent">{result.hosts}</span>
          </div>
        </div>
      ) : (
        <div className="text-center text-sm text-red-400 py-4">Invalid IP Address format.</div>
      )}
    </div>
  );
};
    ...
    <div className="space-y-6">
      <div className="flex space-x-4 border-b border-slate-700 pb-4">
        <button
          ...
          className={`text-sm font-medium pb-1 transition-colors ${mode === 'cidrToMask' ? 'text-brand-accent border-b-2 border-brand-accent' : 'text-brand-muted hover:text-white'}`}
        >
          CIDR to Mask
        </button>
        <button
           onClick={() => { setMode('maskToCidr'); setInput('255.255.255.0'); }}
           className={`text-sm font-medium pb-1 transition-colors ${mode === 'maskToCidr' ? 'text-brand-accent border-b-2 border-brand-accent' : 'text-brand-muted hover:text-white'}`}
        >
          Mask to CIDR
        </button>
      </div>
      ...
      <div className="bg-slate-900 p-6 rounded border border-slate-800 text-center">
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-4">
        <input
            ...
            className="flex-grow bg-slate-800 border border-slate-700 text-white px-4 py-2 rounded focus:border-brand-accent outline-none font-mono"
          />
        <button 
          onClick={runScan}
          disabled={scanning}
      ...
      {/* Terminal Output */}
      <div className="bg-black rounded-lg border border-slate-800 p-4 font-mono text-sm h-64 overflow-y-auto flex flex-col relative">
...
const DnsLookupTool = () => {
  ...
  return (
    <div className="space-y-6">
       <div className="flex flex-col sm:flex-row gap-4">
         <input
          ...
          <select 
            ...
            className="bg-slate-800 border border-slate-700 text-white px-4 py-2 rounded focus:border-brand-accent outline-none"
          >
       ...
       {error && (
         <div className="bg-red-900/20 border border-red-900 text-red-400 p-4 rounded flex items-center space-x-2">
           <AlertCircle size={18} />
           <span>{error}</span>
         </div>
       )}
       ...
       {data && (
         <div className="bg-slate-900 rounded border border-slate-800 overflow-hidden">
           <div className="bg-slate-800 px-4 py-2 border-b border-slate-700 flex justify-between items-center">
             <span className="text-xs font-bold text-slate-400 uppercase">Answer Section</span>
           ...
           {data.Answer ? (
             <div className="overflow-x-auto">
                ...
                <tbody className="divide-y divide-slate-800">
                    ...
                    <tr key={idx} className="hover:bg-slate-800/30 font-mono">
                        <td className="px-4 py-2 text-brand-muted truncate max-w-[120px]" title={record.name}>{record.name}</td>
                        <td className="px-4 py-2 text-white">{type} ({record.type})</td>
                        <td className="px-4 py-2 text-brand-muted">{record.TTL}</td>
             ...
             <div className="p-4 text-brand-muted text-center">No answer records returned from authority.</div>
           )}
         </div>
       )}
    </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      {activeToolId && activeToolDef && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-brand-surface border border-slate-700 rounded-xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center p-4 border-b border-slate-700 bg-slate-900">
              ...
              <button onClick={() => setActiveToolId(null)} className="text-slate-400 hover:text-white transition-colors">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="p-4 bg-slate-900 border-t border-slate-700 flex justify-end">
               <button onClick={() => setActiveToolId(null)} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded text-sm border border-slate-700 transition-colors">
                 Close Session
               </button>
            </div>
          </div>
